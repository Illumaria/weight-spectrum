...
* `-o, --output <output>`: (необязательно) путь к файлу, куда будут сохранены выходные данные; при отсутствии значения выходные данные будут сохранены в файл
                           `output.txt` в текущем каталоге;
* `-j, --parallel <jobs>`: (необязательно) количество процессов (ядер процессора), которое будет задействовано при вычислениях; при отсутствии значения будет
                           использовано максимально возможное количество ядер, полученное автоматически.
